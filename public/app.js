...
async function api(path, options = {}) {
  ...
  // Record every api() outcome into the ring so a feedback report shows what
  // happened just before the user hit "Report a problem". Only the pathname is
  // stored (no query strings / bodies) to keep it non-sensitive and small.
  ...
  try {
    ...
    if (error && error.status === undefined) {
      // fetch itself failed (network/abort) — no HTTP status.
      ringPush(
        recentApiCalls,
        { t: startedAt, path: shortPath, method, status: 0, ms: relNow() - startedAt, ok: false, err: String(error && error.name || "error") },
        RECENT_API_MAX,
      );
    }
    throw error;
