...
export function createTranscriptArchive({
  ...
  async function readEpoch(manifestKey, { manifest: snapshot, signal, after } = {}) {
    ...
    if (storage.listKeys && (!after || manifest.committedOffset - after.committedOffset > 4 * 1024 * 1024)) {
      signal?.throwIfAborted();
      ...
      const keys = (await (signal ? abortable(listing, signal) : listing)).filter(key => {
        ...
        return range && Number(range[2]) <= manifest.committedOffset && Number(range[1]) >= (after?.committedOffset || 0);
      });
    ...
    while (metadataKey && metadataKey !== after?.metadataKey) {
      signal?.throwIfAborted();
    ...
    if (after && metadataKey !== after.metadataKey) throw invalidManifestChain("checkpoint outside committed chain");
    validateReplayContinuity(chunks, manifest, after);
...
function validateReplayContinuity(chunks, manifest, after) {
  let committedOffset = after?.committedOffset || 0;
  let nextLineSeq = after?.nextLineSeq || 0;
