...
export function createTranscriptArchive({
  ...
  async function commitChunkUnlocked({ source, decoded, keys }) {
    ...
    if (
      ...
      const committed = await manifestContainsMetadata(
        storage,
        manifest,
        keys.metadataKey,
      );
      const existing = committed ? await storage.get(keys.chunkKey) : null;
...
async function manifestContainsMetadata(storage, manifest, targetKey) {
  let metadataKey = manifest?.lastChunk?.metadataKey || "";
  const visited = new Set();
  while (metadataKey) {
    if (metadataKey === targetKey) return true;
