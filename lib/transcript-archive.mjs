...
export function createTranscriptArchive({
  ...
  async function readEpoch(manifestKey) {
    ...
    // Raw blocks are independent after the committed chain is validated.
    // Bound parallel reads so opening a long conversation does not serialize
    // another S3 round trip for every block, or flood storage with requests.
    for (let offset = 0; offset < chunks.length; offset += 8) {
      await Promise.all(chunks.slice(offset, offset + 8).map(async metadata => {
