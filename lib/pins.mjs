...
// "org" = everyone in the owner's Google Workspace hosted domain (hd), mirroring
// how machine visibility is shared within a domain. (Consumer accounts have no
// hd, so "org" matches no one for them — effectively private.)
const SHARE_SCOPES = new Set(["private", "users", "org", "all"]);
const DEFAULT_USER = "default";
const viewerStore = new AsyncLocalStorage();

// Super admins see every pin (mirrors lib/hub.mjs canAccessMachine, where they
// see every machine). Set at startup from config/access-control.json.
let superAdmins = new Set();
export function setPinSuperAdmins(emails) {
  superAdmins = new Set(
...
// A short text preview of the content for list UIs, so a row shows what the
// artifact IS rather than an internal source path. Only for text-ish content.
function previewFromBytes(buffer, contentType, kind) {
  const ct = String(contentType || "");
  const isText =
    kind === "markdown" || /^text\//i.test(ct) || /json|xml|yaml|javascript/i.test(ct);
  if (!isText) return "";
  let text;
  try {
    text = buffer.toString("utf8", 0, 4096);
  } catch {
    return "";
  }
  // Light markdown cleanup so the snippet reads as prose, then collapse + clip.
  return text
    .replace(/```[\s\S]*?```/g, " ")
    .replace(/^#{1,6}\s+/gm, "")
    .replace(/!\[[^\]]*\]\([^)]*\)/g, " ")
...
export function canSeePin(viewerInput, pin) {
  ...
  if (superAdmins.has(viewer.email) || superAdmins.has(viewer.userId)) return true; // admin
  switch (pin.share.scope) {
    case "all":
      return true; // any logged-in viewer
    case "org":
      // Same Google Workspace hosted domain as the owner.
      return Boolean(viewer.hd && pin.ownerHd && viewer.hd === pin.ownerHd);
    case "users":
...
export async function createPin(
  ...
  const pin = sanitizeRecord(randomUUID(), {
    ...
    preview: previewFromBytes(buffer, contentType, kind),
    kind: kind || "other",
...
// Owner-only. Rename the artifact's display name (does not change its bytes,
// token, family, or version). Throws 404/403/400 like updateShare.
export async function renamePin(pinId, viewer, name, { now = Date.now } = {}) {
  const pin = await recordById(pinId);
  if (!pin) {
    const e = new Error("Pin not found");
    e.status = 404;
  ...
  if (!isOwner(viewer, pin)) {
    const e = new Error("Only the owner can rename");
    e.status = 403;
    throw e;
  }
  const clean = String(name || "").replace(/[\\/]/g, "").trim().slice(0, 200);
  if (!clean) {
