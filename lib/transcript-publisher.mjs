...
const DEFAULT_MAX_UPLOAD_BYTES_PER_SYNC = 1024 * 1024;
const DEFAULT_MAX_CHUNKS_PER_SESSION_SYNC = 1;
...
export function createAgentTranscriptPublisher({
  ...
  maxUploadBytesPerSync = positiveCountFromEnv(
    process.env.TMUX_MOBILE_TRANSCRIPT_MAX_BYTES_PER_SYNC,
    DEFAULT_MAX_UPLOAD_BYTES_PER_SYNC,
  ),
  maxChunksPerSessionSync = positiveCountFromEnv(
    process.env.TMUX_MOBILE_TRANSCRIPT_MAX_CHUNKS_PER_SESSION_SYNC,
    DEFAULT_MAX_CHUNKS_PER_SESSION_SYNC,
  ),
  ...replicatorOptions
} = {}) {
  ...
  const replicator = createTranscriptReplicator({
    stateStore,
    maxChunksPerSync: maxChunksPerSessionSync,
    ...replicatorOptions,
  ...
  async function tick() {
    ...
    for (const [sessionKey, context] of orderedContexts) {
      // A connector disconnect pauses archive traffic. The in-flight request
      // is aborted by the transport; do not advance to another source while
      // the control channel is recovering.
      if (!enabled || stopped) break;
      if (uploadedBytes >= maxUploadBytesPerSync) {
        deferred += 1;
        continue;
      }
      const active =
      ...
      try {
        ...
        uploadedBytes += result.uploaded.reduce(
          (total, item) => total + Number(item?.bytes?.length || 0),
          0,
        );
