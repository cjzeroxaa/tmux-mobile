...
// Codex approval prompt. Unlike Claude, codex blocks on an approval prompt when
// it wants to edit files / run a command, rendered as a `›`-cursor numbered
// option list with a fixed confirm footer, e.g.:
//   Would you like to make the following edits?
//   › 1. Yes, proceed (y)
//     2. Yes, and don't ask again for these files (a)
//     3. No, and tell Codex what to do differently (esc)
//   Press enter to confirm or esc to cancel
// The footer is the unambiguous high-confidence anchor — codex's IDLE state shows
// a bare `›` placeholder (e.g. "› Implement {feature}") with NO confirm footer,
// so the footer cleanly separates "blocked, needs approval" from "turn ended".
// (Discovered via the attention-watch shadow run: these prompts were being read
// as idle/finished — a false NEGATIVE that hid a blocked agent. See
// docs/DETECTION.md.)
const CODEX_CONFIRM_FOOTER_RE = /Press enter to confirm(?:\s+or esc to cancel)?/i;
