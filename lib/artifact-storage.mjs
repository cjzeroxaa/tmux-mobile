...
function createLocalDriver(env) {
  ...
  return {
    ...
    async listPrefixes(prefix) {
      try {
        return (await readdir(resolveKey(prefix), { withFileTypes: true }))
          .filter(entry => entry.isDirectory()).map(entry => `${prefix}${entry.name}/`);
      } catch (error) { if (error.code === "ENOENT") return []; throw error; }
    },
    // Direct children only: archive ranges are a flat directory.
    async listKeys(prefix) {
      try {
        return (await readdir(resolveKey(prefix), { withFileTypes: true }))
          .filter(entry => entry.isFile()).map(entry => `${prefix}${entry.name}`);
      } catch (error) { if (error.code === "ENOENT") return []; throw error; }
    },
    servesDirectly() {
      return false;
...
async function createS3Driver(env) {
  ...
  return {
    kind: "s3",
    async listPrefixes(keyPrefix, { signal } = {}) {
      const found = [];
      let token;
      do {
        const page = await client.send(new ListObjectsV2Command({ Bucket: bucket,
          Prefix: objectKey(keyPrefix), Delimiter: "/", ContinuationToken: token }), { abortSignal: signal });
        found.push(...(page.CommonPrefixes || []).map(item => item.Prefix.slice(prefix.length)));
        token = page.IsTruncated ? page.NextContinuationToken : undefined;
      } while (token);
      return found;
    },
    async listKeys(keyPrefix, { signal } = {}) {
      const found = [];
      let token;
      do {
        const page = await client.send(new ListObjectsV2Command({ Bucket: bucket,
          Prefix: objectKey(keyPrefix), Delimiter: "/", ContinuationToken: token }), { abortSignal: signal });
        found.push(...(page.Contents || []).map(item => item.Key.slice(prefix.length)));
        token = page.IsTruncated ? page.NextContinuationToken : undefined;
      } while (token);
      return found;
    },
    ...
    async put(key, bytes, { contentType, ifVersion } = {}) {
      const buffer = Buffer.isBuffer(bytes) ? bytes : Buffer.from(bytes);
      try {
        await client.send(
          new PutObjectCommand({
            ...
            ...(ifVersion === null ? { IfNoneMatch: "*" } : {}),
            ...(typeof ifVersion === "string" && ifVersion
              ? { IfMatch: ifVersion }
              : {}),
          }),
        );
        ...
        if (
          ...
          throw storageVersionConflict(error);
        }
        throw error;
      }
      return { key, size: buffer.length };
    ...
    async get(key, { signal } = {}) {
      try {
        ...
        const abortBody = () => res.Body.destroy?.(signal.reason);
...
async function createGcsDriver(env) {
  ...
  const fileFor = (key, options) => bucket.file(`${prefix}${key}`, options);
  const presign = presignEnabled(env);
  return {
    kind: "gcs",
    async listPrefixes(keyPrefix) {
      const found = [];
      let query = { prefix: `${prefix}${keyPrefix}`, delimiter: "/", autoPaginate: false };
      do {
        const [, next, response] = await bucket.getFiles(query);
        found.push(...(response.prefixes || []).map(item => item.slice(prefix.length)));
        query = next;
      } while (query);
      return found;
    },
    async listKeys(keyPrefix) {
      ...
      do {
        ...
        found.push(...files.map(file => file.name.slice(prefix.length)));
        query = next;
      } while (query);
      return found;
    },
        ...
        await fileFor(key).save(buffer, {
          ...
          ...(ifVersion !== undefined
            ? {
                preconditionOpts: {
                  ifGenerationMatch:
                    ifVersion === null ? 0 : ifVersion,
                },
              }
            : {}),
        });
      } catch (error) {
        if (ifVersion !== undefined && Number(error?.code) === 412) {
          throw storageVersionConflict(error);
        }
        throw error;
      }
    ...
    async get(key, { withVersion = false } = {}) {
      try {
        if (!withVersion) {
          const [bytes] = await fileFor(key).download();
          return { bytes, size: bytes.length };
        }
        const [metadata] = await fileFor(key).getMetadata();
        const version = String(metadata.generation || "");
        const [bytes] = await fileFor(
          key,
          version ? { generation: version } : undefined,
        ).download();
        return { bytes, size: bytes.length, version: version || null };
      } catch (error) {
...
function storageVersionConflict(cause) {
  const error = new Error("Object storage version precondition failed");
