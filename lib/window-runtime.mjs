...
export function createWindowRuntime(backend) {
  return new TmuxWindowRuntime(backend, backendMuxKind(backend));
}
...
class TmuxWindowRuntime {
  ...
  commandName() {
    return String(backendValue(this.backend, "muxCommand", this.kind) || this.kind);
  }
