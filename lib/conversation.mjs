...
export function createConversationReader({ storage, archive, maxCacheBytes = 32 * 1024 * 1024 }) {
  const cache = new Map();
  ...
  async function read(source) {
    ...
    const operation = (async () => {
      const epochs = await storage.listPrefixes(prefix);
      const manifests = [];
      ...
      const epoch = await archive.readEpoch(latest.key);
