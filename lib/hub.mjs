...
export function createHub(
  ...
  // Total agents connected RIGHT NOW, across all owners (no viewer scoping).
  // Used by /api/health so a deploy can tell when an agent has (re)connected to
  // a freshly-rolled-out revision before shifting traffic to it.
  function connectedCount() {
    return machines.size;
  }

  // Ask every connected agent to reconnect now (graceful deploy handoff). The
  // old instance lingers holding these sockets after a rollout; pushing REDIAL
  // makes each agent re-dial the main URL immediately (landing on the new
  // revision) instead of waiting to notice the change by polling. Returns how
  // many agents were signalled.
  function redialAgents() {
    let n = 0;
    const frame = JSON.stringify({ t: MSG.REDIAL });
    for (const machine of machines.values()) {
      if (machine.ws?.readyState === machine.ws?.OPEN) {
        try {
          machine.ws.send(frame);
          n += 1;
        } catch {}
      }
    }
