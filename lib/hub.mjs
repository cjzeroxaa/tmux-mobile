...
export function createHub(
  ...
  function machineFor(viewerInput, machineId) {
    ...
    const matches = visibleMachines(viewerInput).filter((machine) => machine.machineId === id);
  ...
  function listMachines(viewer = "") {
    return visibleMachines(viewer)
      .map((machine) => {
        ...
        const revisionState = agentRevisionState(machine.info?.revision, currentRevision);
        return {
          id: machine.id,
          ...
          // Stale = older protocol surface or older/unknown code revision.
          stale: missingOps.length > 0 || revisionState.stale,
          missingOps,
          revisionStatus: revisionState.status,
        };
      });
  }
...
function agentRevisionState(agentRevision, expectedRevision) {
  const expected = normalizeRevision(expectedRevision);
  if (!expected) return { stale: false, status: "unverified" };
  const agent = normalizeRevision(agentRevision);
  if (!agent) return { stale: true, status: "missing" };
  if (revisionsMatch(agent, expected)) return { stale: false, status: "current" };
  return { stale: true, status: "outdated" };
}

function normalizeRevision(value) {
  const revision = String(value || "").trim();
  if (!revision || revision === "dev") return "";
  return revision;
}

function revisionsMatch(agentRevision, expectedRevision) {
  if (agentRevision === expectedRevision) return true;
  if (agentRevision.includes("-dirty") || expectedRevision.includes("-dirty")) return false;
  if (agentRevision.length >= 7 && expectedRevision.startsWith(agentRevision)) return true;
  if (expectedRevision.length >= 7 && agentRevision.startsWith(expectedRevision)) return true;
  return false;
}
