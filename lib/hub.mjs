...
export function createHub(
  ...
  // Open-team mode: every signed-in user can see every connected machine,
  // regardless of who registered it. The ownerId argument is kept so the
  // call sites in server.mjs don't churn (and so we can re-introduce
  // per-group filtering later without changing them), but it's no longer
  // consulted on viewer-side lookups. The Map is still composite-keyed
  // by (ownerId, machineId) so two users' agents that happen to share a
  // hostname-derived machineId stay distinct entries; we just look them
  // up by machineId across all values now.
  ...
  function rpc(_ownerId, machineId, op, payload) {
  ...
  function listMachines(_ownerId = "") {
    // Open-team: return every connected machine to every signed-in user.
    // ownerId is included in the per-machine entry so the UI can label
    // whose box it is.
    return [...machines.values()].map((machine) => {
      ...
      return {
        ...
        // Who registered it (their Google email). Exposed so the UI can
        // tag each machine with the contributor; the viewer's own email
        // comes from the cookie on their side.
  ...
  function hasMachine(_ownerId = "", machineId) {
    return findMachine(machineId) !== null;
  }
  ...
  function agentSupportsOp(_ownerId = "", machineId, op) {
    const machine = findMachine(machineId);
  ...
  function soleMachineId(_ownerId = "") {
    const all = [...machines.values()];
    return all.length === 1 ? all[0].machineId : "";
  }
