...
export function createHub(
  ...
  const machines = new Map(); // route id -> { owner, ownerId, agentId, machineId, ws, info, pending, lastSeen }
  ...
  function describeMachine(machine) {
    ...
    const inventory = describeInventory(machine);
    return {
      id: machine.id,
...
function updateInventory(machine, msg) {
  const observedAt = normalizeTimestamp(msg.observedAt) || Date.now();
  const previousAgents = Array.isArray(machine.inventory?.agents)
...
function describeInventory(machine) {
  ...
  const ageMs = Math.max(0, Date.now() - inventory.observedAt);
  const status = inventory.ok
    ? ageMs > INVENTORY_STALE_MS
      ? "stale"
      : "fresh"
    : "failed";
  return {
