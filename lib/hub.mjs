...
export function createHub(
  ...
  const pingTimer = setInterval(() => {
    for (const machine of machines.values()) {
      ...
      // Command Center inventory is observability, not transport liveness. A
      // slow tmux/rmux scan must not make send/read RPCs report the whole
      // machine offline.
      try {
        machine.ws.ping();
      } catch {}
  ...
  function backendFor(viewer, machineId) {
    ...
    return {
      ...
      async rmuxWebShare({ target, ttlSeconds, tunnelProvider } = {}) {
