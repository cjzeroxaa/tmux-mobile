...
// Revision migration: on a controller deploy the agent's live WebSocket can keep
// the OLD instance alive, so the controller may not close us promptly and the
// agent would stay pinned to stale code indefinitely. So the agent itself polls
// the controller's /api/health revision; when it changes from the one we
...
// `/api/health` is load-balanced separately from the live WebSocket. During an
// ECS rolling deploy, it can briefly alternate between old and new tasks. Require
// a new revision to be observed in consecutive polls before migrating the socket.
const REVISION_STABLE_POLLS = Math.max(
  1,
  Number(process.env.AGENT_REVISION_STABLE_POLLS) || 3,
);
...
export function runAgent(
  ...
  async function describeMux() {
    ...
    try {
      return {
        mux,
        muxCommand,
        version: (await backend.tmux(["-V"])).trim(),
      };
    } catch {
      return { mux, muxCommand, version: "" };
    }
  }
  ...
  function connect() {
    ...
    function setConnectionRevision(revision) {
      const value = String(revision || "").trim();
      if (!value) return;
      connectionRevision = value;
      seenRevisions.add(value);
      pendingRevision = "";
      pendingRevisionCount = 0;
    }

    function maybeMigrateRevision(current) {
      if (!current) return;
      if (!connectionRevision) {
        setConnectionRevision(current);
        return;
      }
      if (current === connectionRevision) {
        pendingRevision = "";
        pendingRevisionCount = 0;
        return;
      }
      if (seenRevisions.has(current)) {
        pendingRevision = "";
        pendingRevisionCount = 0;
        return;
      }
      ...
      pendingRevisionCount += 1;
      if (pendingRevisionCount < REVISION_STABLE_POLLS) return;
      logEvent("agent_revision_changed", {
        controller: new URL(hubUrl).origin,
        machine: machineName,
        from: connectionRevision,
        to: current,
        stablePolls: pendingRevisionCount,
        message:
          "Controller deployed a stable new revision; reconnecting to move off the old instance.",
      });
    ...
    ws.on("open", async () => {
      ...
      livenessTimer = setInterval(() => {
        ...
        if (Date.now() - lastPong > PONG_TIMEOUT_MS) {
          logEvent("agent_connection_dead", {
            ...
            machine: machineName,
            message:
              "Controller stopped responding to pings; terminating the dead socket to force a reconnect.",
          });
          ws.terminate();
          return;
        }
      ...
      // The controller sends an INFO frame with the revision for this WebSocket's
      // actual instance. The health fetch below is only a fallback for older hubs
      // and never blocks hello registration.
      if (REVISION_POLL_MS > 0) {
        fetchRevision().then((rev) => {
          if (rev && !connectionRevision) setConnectionRevision(rev);
        });
        revisionTimer = setInterval(async () => {
          if (ws.readyState !== ws.OPEN) return;
          const current = await fetchRevision();
          maybeMigrateRevision(current);
        }, REVISION_POLL_MS);
        revisionTimer.unref?.();
      }
      ...
      stopInventoryPublisher = startInventoryPublisher(ws, inventoryProvider, {
        hubUrl,
        machineName,
        agentId,
        logEvent,
    ...
    ws.on("message", async (raw) => {
      ...
      if (msg.t === MSG.INFO) {
        setConnectionRevision(msg.revision);
        return;
      }
    ...
    ws.on("close", (code) => {
      ...
      // The controller accepted another connector for the same canonical
      // machine. This process is stale; reconnecting would fight the replacement.
      if (code === AGENT_CLOSE_CODE.REPLACED) {
        stopped = true;
        logEvent("agent_stopped", {
          controller: new URL(hubUrl).origin,
          machine: machineName,
      ...
      const intentionalRestart = code === AGENT_CLOSE_CODE.CONTROLLER_RESTARTING;
...
function startInventoryPublisher(
  ...
  async function tick() {
    if (stopped || inFlight || ws.readyState !== ws.OPEN) return;
