...
export function runAgent(hubUrl, backend, { logEvent = () => {} } = {}) {
  ...
  // Identity precedence: explicit AGENT_MACHINE env (lets you override) > the
  // name persisted on a previous run > os.hostname() (first-ever run). Whatever
  // we resolve is persisted so subsequent plain restarts keep the SAME identity
  // and don't strand sessions waiting on the old name.
  const machineName =
    process.env.AGENT_MACHINE || loadStoredMachine() || os.hostname();
