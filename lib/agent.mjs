...
// Single source of truth for this machine's agent identity. Precedence:
// explicit AGENT_MACHINE env (override) > the name persisted on a previous run >
// os.hostname() (first-ever run). Used by both runAgent and the startup log so
// they never disagree.
export function resolveMachineName() {
  return process.env.AGENT_MACHINE || loadStoredMachine() || os.hostname();
}

export function runAgent(hubUrl, backend, { logEvent = () => {} } = {}) {
  const wsUrl = toWsUrl(hubUrl);
  const machineName = resolveMachineName();
  // Persist whatever we resolved so subsequent plain restarts keep the SAME
  // identity and don't strand sessions waiting on the old name.
  saveStoredMachine(machineName); // fire-and-forget; best-effort
  ...
  function connect() {
    ...
    ws.on("message", async (raw) => {
      ...
      // Graceful deploy handoff: the controller asks us to reconnect NOW (e.g.
      // it just rolled out a new revision and shifted traffic). Terminate to
      // trigger the existing close→reconnect path, which re-dials the main URL —
      // now serving the new revision. This makes the migration immediate instead
      // of waiting up to REVISION_POLL_MS to notice on our own.
      if (msg.t === MSG.REDIAL) {
        logEvent("agent_redial_requested", {
          controller: new URL(hubUrl).origin,
          machine: machineName,
          message:
            "Controller requested a reconnect (graceful deploy handoff); reconnecting now.",
        });
        ws.terminate();
        return;
      }
...
// The machine name persists across restarts so a plain restart (no AGENT_MACHINE
// in the env) keeps the SAME identity — otherwise it would fall back to a
// different os.hostname() and strand browser sessions waiting on the old name.
function loadStoredMachine() {
  try {
    const config = JSON.parse(readFileSync(agentConfigPath(), "utf8") || "{}");
    return typeof config.machine === "string" ? config.machine.trim() : "";
  } catch {
    return "";
  }
}

async function saveStoredMachine(machine) {
  const name = String(machine || "").trim();
  if (!name) return;
  try {
    const filePath = agentConfigPath();
    const config = await readAgentConfig();
    if (config.machine === name) return; // already persisted; no write
    config.machine = name;
    await mkdir(path.dirname(filePath), { recursive: true, mode: 0o700 });
    await writeFile(filePath, `${JSON.stringify(config, null, 2)}\n`, { mode: 0o600 });
  } catch {
    // Best-effort: a read-only home shouldn't stop the agent from running. It
    // just means the name won't be sticky on the next restart.
  }
}
