...
export function muxCommandFromEnv(env = process.env) {
  const raw = String(env.TMUX_MOBILE_MUX_COMMAND || env.TMUX_MOBILE_MUX || "tmux").trim();
  if (!raw) return "tmux";
...
export const localBackend = {
  ...
  muxCommand() {
    return muxCommandFromEnv();
  },
  muxKind() {
    return muxKindFromCommand(this.muxCommand());
  },
  tmux(args, options = {}) {
    const command = this.muxCommand();
