...
import { normalizeAgentSessionTitle } from "./agent-session-title.mjs";
...
export async function findClaudeSessionFromBackend(backend, arg) {
  ...
  for (const pid of candidates) {
    ...
    return {
      ...
      agentSessionTitle: normalizeAgentSessionTitle(sessionInfo?.name),
      agentSessionTitleSource: sessionInfo?.name
        ? `claude-name${sessionInfo?.nameSource ? `:${sessionInfo.nameSource}` : ""}`
        : "",
    };
  }
  return null;
...
export async function readClaudeTranscriptFromSession(backend, session, { maxBytes = TRANSCRIPT_TAIL_BYTES } = {}) {
  ...
  if (backend === localBackend) {
    ...
    return {
      ...
      transcriptPath,
      agentSessionTitle: session.agentSessionTitle || "",
      agentSessionTitleSource: session.agentSessionTitleSource || "",
      turns: parsed.turns,
      turnsTotal: parsed.total,
    };
  }
