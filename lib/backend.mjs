...
export const localBackend = {
  ...
  // The directory new worktrees go under for the repo containing `dirPath`.
  // Authoritative (no heuristics): git --git-common-dir resolves to the shared
  // repo all worktrees share — for the bare-repo layout that's the `.bare` dir,
  // and worktrees live as siblings of it, so the root is its PARENT. e.g.
  //   /home/ubuntu/worktrees/kernel/.bare  ->  /home/ubuntu/worktrees/kernel
  // A new branch's worktree is then exactly <root>/<branch> (slashes preserved,
  // matching how every existing worktree is laid out). Returns "" on failure.
  async worktreeRoot(dirPath) {
    try {
      const out = await execFileAsync(
        "git",
        ["-C", dirPath, "rev-parse", "--path-format=absolute", "--git-common-dir"],
        { timeout: 4000 },
      );
      const commonDir = out.trim();
      if (!commonDir) return "";
      return path.dirname(commonDir.replace(/\/+$/, ""));
    } catch {
      return "";
    }
  },
  // Create a new branch in a new git worktree at <worktreeRoot>/<branch>, the
  // canonical location for the bare-repo layout (next to .bare, branch path
  // preserved). Returns { path, branch, root }. Throws on a bad branch name, a
  // non-worktree source, or a git failure (branch exists, dir occupied, …).
  async worktreeAdd({ fromDir, branch } = {}) {
    const name = String(branch || "").trim();
    // Safe ref subset; reject anything that could escape the dir or confuse git/
    // the shell. No leading dash, no "..", no leading/trailing/double slash, no
    ...
    const root = await this.worktreeRoot(base);
    if (!root) {
      const error = new Error("could not resolve the worktree root (not a git worktree?)");
      error.status = 400;
      throw error;
    }
    const target = path.join(root, name); // <root>/<branch>, slashes preserved
    await execFileAsync(
      "git",
      ["-C", base, "worktree", "add", "-b", name, target],
      { timeout: 20000 },
    );
    return { path: target, branch: name, root };
  },
  /**
