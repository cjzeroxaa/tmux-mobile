...
export function createTranscriptReplicator({
  ...
  async function syncUnlocked({ sessionKey, filePath }) {
    ...
    async function recoverRemoteConflict(error) {
      if (!isRemoteCursorConflict(error)) throw error;
      if (!state?.pending) throw error;
      if (recoveredRemoteConflict || state.remoteRecovery) {
        return quarantinePending(
          error,
          "recovery-epoch-rejected",
          "transcript_pending_remote_conflict",
        );
      }

      const previousState = state;
      const previousPending = cloneJson(previousState.pending);
      const metadata = previousPending.metadata;

      // A remote namespace reset requires a new epoch beginning at byte/line
      // zero. Only a zero-based pending chunk contains the complete prefix
      // needed for that replay. For a later pending chunk, the ACKed prefix is
      // not present in the durable pending spool; re-reading the path and then
      // clearing pending has an unavoidable rotation race. Preserve those bytes
      // and block instead of risking silent loss.
      if (
        metadata.startOffset !== 0 ||
        metadata.firstLineSeq !== 0 ||
        metadata.previousChunkSha256
      ) {
        return quarantinePending(error, "pending-prefix-not-spooled");
      }
      ...
      const fileEpoch = String(
        await createFileEpoch({
          sessionKey: key,
          filePath: sourcePath,
          identity: previousState.sourceIdentity,
          reason,
          previousEpoch: previousState.fileEpoch,
        }),
      );
      ...
      // Re-key the exact persisted bytes before retrying. This recovery no
      // longer depends on the current path, so a rename/replacement at any point
      // after the NACK cannot discard the old identity's pending prefix.
      const replayMetadata = {
        ...metadata,
        fileEpoch,
        previousChunkSha256: "",
      };
      replayMetadata.chunkId = pendingChunkId(replayMetadata);
      state = freshState({
        sessionKey: key,
        filePath: sourcePath,
        identity: previousState.sourceIdentity,
      ...
      state.remoteRecovery = {
        code: error.code,
        previousEpoch: previousState.fileEpoch,
        previousChunkId: metadata.chunkId,
      };
      await save(key, state);
    ...
    async function attemptPendingWithRecovery() {
      try {
        return { attempt: await tryPending(key, state), quarantined: false };
      } catch (error) {
        if (!(await recoverRemoteConflict(error))) {
          return { attempt: null, quarantined: true };
        }
      }
    ...
    if (state?.pending) {
      const recovered = await attemptPendingWithRecovery();
      if (recovered.quarantined) return quarantineResult();
